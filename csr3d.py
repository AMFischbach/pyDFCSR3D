...
class CSR3D:
    """
    Class to compute 3D CSR wake, uses adapative meshes at each time step to best fit the beam distribution
    """
    def __init__(self, input_file, parallel = False):
        ...
        print("CSR3D Object Succesfully Initialized")
    ...
    def run(self):
        ...
        for step_index in tqdm(range(1, self.df_tracker.total_steps)):
            ...
            if self.lattice.compute_CSR[step_index] and self.csr_params["compute_CSR"]:
                ...
                dE_vals, x_kick_vals, y_kick_vals = self.compute_CSR_on_mesh(CSR_mesh, formation_length, step_index)
                self.wake_history["dE_history"][step_index-1] = dE_vals
                self.wake_history["x_kick_history"][step_index-1] = x_kick_vals
                self.wake_history["y_kick_history"][step_index-1] = y_kick_vals
                ...
                if self.csr_params["apply_CSR"]:
                    ...
                    self.beam.apply_wakes(dE_vals, x_kick_vals, y_kick_vals, csr_params, t2_csr, R_inv_csr, d_step, p_sd, o_sd)
    ...
    def compute_CSR_on_mesh(self, CSR_mesh, formation_length, step_index):
        ...
        # For each point on the CSR_mesh, compute dE, x_kick, & y_kick and update their respective arrays
        for index in range(len(X)):
            ...
            (dE_vals[index], 
                x_kick_vals[index], 
                y_kick_vals[index]) = self.compute_CSR_at_point(s, x, formation_length, slope, x_mean, x_std, z_std, step_index)
        ...
        y_kick_vals = y_kick_vals.reshape(original_shape)

        return dE_vals, x_kick_vals, y_kick_vals
    ...
    def compute_CSR_at_point(self, s, x, formation_length, slope, x_mean, x_std, z_std, step_index):
        ...
        # Initialize dE, x_kick, and y_kick, the CSR wake perturbation at this point
        dE = 0
        x_kick = 0
        y_kick = 0
        ...
        for area in integration_areas:
            integrand_z, integrand_x, integrand_y = self.get_csr_integrands(s, x, area)
            ...
            y_kick += self.CSR_scaling * np.trapz(y=np.trapz(y=integrand_y, x=area[0][:, 0], axis=0), x=area[1][0,:])

        return dE, x_kick, y_kick
    ...
    def get_integration_areas(self, s, x, formation_length, slope, x_mean, x_std, z_std):
        ...
        """
        #slope = self.beam.linear_fit.slope
        ...
        if sufficent_tilt:
            ...
            xp_w = np.linspace(x1_w, x2_w, self.integration_params["xbins"])
            xp_n = np.linspace(x1_n, x2_n, 2*self.integration_params["xbins"])
    ...
    def get_csr_integrands(self, s, x, area):
        """
        ...
        r_minus_rp = np.sqrt(r_minus_rp_x**2 + r_minus_rp_y**2)                                     # r-r' in the X and Y coordinate of the lab frame
        ...
        interp_data = (self.df_tracker.xz_density, 
        ...
        # COMPUTE Y INTEGRALS
        ############################################
        # Define the sample mesh
        min_t_ret = np.min(t_ret)
        max_t_ret = np.max(t_ret)

        # Populate t_ret and the corresponding D2 points
        sample_t_ret = np.linspace(min_t_ret, max_t_ret, 200)
        sample_D2 = (t - sample_t_ret) * (t - sample_t_ret)

        # Delta y and (delta y)^2 on the y' integration region
        delta_y_1 = self.df_tracker.h_coords_y
        delta_y_2 = self.df_tracker.h_coords_y ** 2

        # Compute two components of the y' integrands at each point on the sample mesh
        y_density_sample, r_minus_rp_3D_sample = get_csr_y_integrand(sample_t_ret,
                                                                     sample_D2,
                                                                     self.df_tracker.y_density,
                                                                     delta_y_2,
                                                                     self.lattice.step_size)

        # Integrated the various y' integrals at all points on the t_ret sample mesh
        y_integrand_sample_x3 = y_density_sample/(r_minus_rp_3D_sample)
        y_integrand_sample_x2 = y_integrand_sample_x3/(r_minus_rp_3D_sample)
        y_integrand_sample_x1 = y_integrand_sample_x2/(r_minus_rp_3D_sample)

        y_integrand_sample_y1 = y_integrand_sample_x1 * delta_y_1
        y_integrand_sample_y2 = y_integrand_sample_x2 * delta_y_1
        
        y_integral_sample_x3 = np.trapz(y_integrand_sample_x3, self.df_tracker.h_coords_y, axis=1)
        y_integral_sample_x2 = np.trapz(y_integrand_sample_x2, self.df_tracker.h_coords_y, axis=1)
        y_integral_sample_x1 = np.trapz(y_integrand_sample_x1, self.df_tracker.h_coords_y, axis=1)
        y_integral_sample_y1 = np.trapz(y_integrand_sample_y1, self.df_tracker.h_coords_y, axis=1)
        y_integral_sample_y2 = np.trapz(y_integrand_sample_y2, self.df_tracker.h_coords_y, axis=1)

        # Create the interpolation function and interpolate all y' integrals at each point on the csr xz mesh
        y_interp_func_x3 = interp1d(sample_t_ret, y_integral_sample_x3, kind='linear')
        y_integrand_x3 = y_interp_func_x3(t_ret)

        y_interp_func_x2 = interp1d(sample_t_ret, y_integral_sample_x2, kind='linear')
        y_integrand_x2 = y_interp_func_x2(t_ret)

        y_interp_func_x1 = interp1d(sample_t_ret, y_integral_sample_x1, kind='linear')
        y_integrand_x1 = y_interp_func_x1(t_ret)

        y_interp_func_y1 = interp1d(sample_t_ret, y_integral_sample_y1, kind='linear')
        y_integrand_y1 = y_interp_func_y1(t_ret)

        y_interp_func_y2 = interp1d(sample_t_ret, y_integral_sample_y2, kind='linear')
        y_integrand_y2 = y_interp_func_y2(t_ret)
        ...
        WZ1 =  scale_term * (C1x*partial_density_ret_lab_x + C1y*partial_density_ret_lab_y) * y_integrand_x3

        # "Integrand of 2nd integral in the z wake"
        WZ2 = - scale_term * beta_dot_beta_ret * density_ret * divergence_velocity * y_integrand_x3
        ...
        # part1: (n - n')(r-r')
        part1 = (n_minus_np_x * r_minus_rp_x) + (n_minus_np_y * r_minus_rp_y)
        ...
        WX1 = scale_term * part1 * density_ret * y_integrand_x1
        WX2 = scale_term * part1 * partial_density * y_integrand_x2
        WX3 = - scale_term * part2 * partial_density * y_integrand_x3
        CSR_integrand_x = WX1 + WX2 + WX3
        ############################################

        # TRANSVERSE WAKE - Y
        ############################################
        tau_dot_tau_p = tau_vec_s_x * tau_vec_sp_x + tau_vec_s_y * tau_vec_sp_y
        part1 = density_ret * (1 - tau_dot_tau_p)
        part2 = partial_density * (1 - tau_dot_tau_p)

        WY1 = scale_term * part1 * y_integrand_y1
        WY2 = scale_term * part2 * y_integrand_y2
        CSR_integrand_y = WY1 + WY2
        ############################################

        # Reshape the integrand values to match the shape of the inputed integration area
        CSR_integrand_x = CSR_integrand_x.reshape(area[1].shape)
        CSR_integrand_z = CSR_integrand_z.reshape(area[1].shape)
        CSR_integrand_y = CSR_integrand_y.reshape(area[1].shape)

        return CSR_integrand_z, CSR_integrand_x, CSR_integrand_y
    ...
    def save_data(self, directory="", filename=""):
        ...
        if self.csr_params["write_wakes"]:
            with h5py.File(file_path_wakes, 'w') as f:
                ...
                g.create_dataset("csr_mesh_coords", data=self.df_tracker.csr_coords_xz[1:])
                ...
                g.create_dataset("y_kick_history", data=self.wake_history["y_kick_history"])
            
    def dump_beam(self, step_index, directory="", filename=""):
        """
        Record the current beam in the particle_group format
        """
        if not filename:
            filename = str(step_index) + ".h5"
