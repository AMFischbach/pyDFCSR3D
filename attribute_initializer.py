...
def init_attributes(input_dict = {}):
    ...
    df_tracker = DF_Tracker(lattice.total_steps, histo_mesh_params, csr_params, beam)
...
def init_histo_params(pbins=100, obins=100, ybins=100, plim=5, olim=5, ylim=5, filter_order=1, filter_window=5, velocity_threshold=1000):
    """
    Initialize the histogram parameters, this dictionary is fed into the step snapshot initalizer
    """
    keys = ["pbins", "obins", "ybins", "plim", "olim", "ylim", "filter_order", "filter_window", "velocity_threshold"]
    values = [pbins, obins, ybins, plim, olim, ylim, filter_order, filter_window, velocity_threshold]

    histo_mesh_params = {k: v for k, v in zip(keys, values)}

    return histo_mesh_params

def init_integration_params(n_formation_length = 4, zbins = 200, xbins = 200, ybins=100):
    """ Initializes the integration_params dictionary"""

    keys = ["n_formation_length", "zbins", "xbins", "ybins"]
    values = [n_formation_length, zbins, xbins, ybins]
...
def init_CSR_params(workdir = '.', apply_CSR = 1, compute_CSR = 1, transverse_on = 1, 
                    pbins = 20, obins = 30, ybins=20, plim = 5, olim = 5, ylim = 5,
                    write_beam = None, write_wakes = True, write_name = ''):
    """ Initializes the CSR_params dictionary"""

    keys = ["workdir", "apply_CSR", "compute_CSR", "transverse_on", "pbins", "obins", "ybins", "plim",
            "olim", "ylim", "write_beam", "write_wakes", "write_name"]
    values = [full_path(workdir), apply_CSR, compute_CSR, transverse_on, pbins, obins, ybins, plim, 
              olim, ylim, write_beam, write_wakes, write_name]
...
def init_wake_history(total_step_num, csr_mesh_params):
    ...
    y_kick_history = np.zeros((total_step_num-1, obins, pbins), dtype=np.float64)

    # Create dictionary
    wake_history = {"dE_history": dE_history,
                    "x_kick_history": x_kick_history,
                    "y_kick_history": y_kick_history}
