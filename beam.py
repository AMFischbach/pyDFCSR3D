...
class Beam():
    ...
    def __init__(self, input_beam):
        ...
        # Record our inital ymean and ystd (used for y histogram mesh)
        self.init_mean_y = np.mean(self.particle.y)
        self.init_std_y = np.std(self.particle.y)
    ...
    def apply_wakes(self, dE_vals, x_kick_vals, y_kick_vals, csr_params, t2_csr, R_inv_csr, step_size, p_sd, o_sd):
        """
        ...
        # Compute the particles new momentum with the kick applied
        pz_new = self.compute_new_p(dE_vals, o_dim, p_dim, step_size, particle_positions, self.particle.pz)
        px_new = self.compute_new_p(x_kick_vals, o_dim, p_dim, step_size, particle_positions, self.particle.px)
        py_new = self.compute_new_p(y_kick_vals, o_dim, p_dim, step_size, particle_positions, self.particle.py)

        # Update the particle object with the new energy and momentum values
        self.particle = Particle(self.particle.x, px_new,
                                 self.particle.y, py_new,
                                 self.particle.z, pz_new,
                                 self.particle.s, self.particle.p0c, self.particle.mc2)

        self.update_status()

    def compute_new_p(self, wake_vals, o_dim, p_dim, step_size, particle_positions, old_p):
        """
        ...
        kick_vals = step_size * wake_vals * 1e6 / self.init_energy

        # Create an interpolator that will transfer the CSR wake from the CSR mesh to the beam's particles
        kick_interp = RegularGridInterpolator((o_dim, p_dim), kick_vals, fill_value=0.0, bounds_error=False)

        # Apply the interpolator to populate the change in momentum for all particles in the beam
        kick_per_particle = kick_interp(particle_positions)

        # Apply kick, note that since the electrons are moving at near the speed of light,
        # change in momentum is roughly equal to change in energy
        p_new = old_p + kick_per_particle
